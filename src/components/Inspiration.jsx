...
function AddInspirationForm({ onSave, onClose }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (!photo) { setError('Please upload an image'); return }
    if (!occasion) { setError('Please select an occasion'); return }
    setLoading(true)
    onSave({ id: uuidv4(), photo, occasion, addedAt: Date.now() })
    setLoading(false)
  }
    ...
    <div className={styles.page}>
      ...
      {showAdd && (
        <Modal title="Add Inspiration" onClose={() => setShowAdd(false)}>
          <AddInspirationForm onSave={handleSave} onClose={() => setShowAdd(false)} />
        </Modal>
      )}
    </div>
  )
